				...
				public override bool ObjectIsValidForTargeting
				{
					 get
					 {
						  ...
						  if (this.targetType.Value==TargetType.Item)
						  {
								...
								double lootDistance=(iCurrentMaxLootRadius+SettingsFunky.ItemRange)*dMultiplier;
						  ...
						  else
						  {
								...
								else
								{
									 ...
									 if (this.CentreDistance>iCurrentMaxLootRadius||this.CentreDistance>37f)
									 {
										  this.BlacklistFlag=BlacklistType.Temporary;
										  this.NeedsRemoved=true;
										  return false;
				...
				public override bool UpdateData()
				{
					 ...
					 if (this.targetType.Value==TargetType.Item)
					 {
						  ...
						  if (!this.DynamicID.HasValue)
						  {
								...
								} catch (Exception ex) { Logging.WriteVerbose("Failure to get Dynamic ID for {0} \r\n Exception: {1}", this.InternalName, ex.Message); return false; }
						  }
						  #endregion
						  ...
						  if (!this.BalanceID.HasValue)
						  {
								...
								} catch { Logging.WriteVerbose("Failure to get gamebalance ID for item {0}", this.InternalName); return false; }
						  }
						  ...
						  if (!this.Itemquality.HasValue||this.ItemQualityRechecked==false)
						  {
								...
								} catch { Logging.WriteVerbose("Failure to get item quality for {0}", this.InternalName); return false; }
					 ...
					 else
					 {
						  ...
						  if (!this.GoldAmount.HasValue)
						  {
								...
								} catch { Logging.WriteVerbose("Failure to get gold amount for gold pile!"); return false; }
						  }
				...
				public override RunStatus Interact()
				{
					 ...
					 if (Bot.Character.FreeBackpackSlots<=8)
					 {
						  if (this.ref_DiaItem!=null&&this.ref_DiaItem.BaseAddress!=IntPtr.Zero)
						  {
								...
								{
									 DbHelper.Log(DbHelper.TrinityLogLevel.Debug, DbHelper.LogCategory.Behavior, "Safely handled exception using PickUpItem(ACDITEM)!");
								}

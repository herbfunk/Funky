	 ...
	 public partial class Funky
	 {
		  public class CacheGizmo : CacheObject
		  {
				...
				public override bool ObjectIsValidForTargeting
				{
					 get
					 {
						  ...
						  if (centreDistance>iCurrentMaxLootRadius)
						  {
				...
				public override RunStatus Interact()
				{
					 if (Bot.Combat.powerPrime.Power!=SNOPower.None)
					 {
						  ...
						  {//Melee class and no change in target.. we attempt to attack by shift-clicking..
								...
								ZetaDia.Me.UsePower(Bot.Combat.powerPrime.Power, vAttackPoint, iCurrentWorldID, -1);
						  }
						  else
						  {
								// Standard attack - attack the ACDGUID (equivalent of left-clicking the object in-game)
								ZetaDia.Me.UsePower(Bot.Combat.powerPrime.Power, vNullLocation, iCurrentWorldID, base.AcdGuid.Value);
						  }
		  ...
		  public class CacheInteractable : CacheGizmo
		  {
				...
				public override RunStatus Interact()
				{
					 ...
					 ZetaDia.Me.UsePower(SNOPower.Axe_Operate_Gizmo, base.Position, iCurrentWorldID, base.AcdGuid.Value);
					 this.InteractionAttempts++;

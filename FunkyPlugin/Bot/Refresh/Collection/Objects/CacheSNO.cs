		  ...
		  public class CachedSNOEntry : SNO
		  {
				...
				public virtual bool UpdateData(DiaObject thisObj, int raguid)
				{
					 ...
					 if (this.InternalName==null)
					 {
						  ...
						  } catch { Logging.WriteVerbose("Failure to get internal name on object, SNO {0}", this.SNOID); return false; }
					 }
					 ...
					 if (!this.Actortype.HasValue)
					 {
						  ...
						  } catch { Logging.WriteVerbose("Failure to get actorType for object, SNO: {0}", this.SNOID); return false; }
						  #endregion
					 }

					 //Ignored actor types..
					 if (dbRefresh.IgnoredActorTypes.Contains(this.Actortype.Value))//||!LootBehaviorEnabled&&this.Actortype.Value==ActorType.Item)
					 {
						  dbRefresh.IgnoreThisObject(this, raguid, true, true);
						  return false;
					 }
						  ...
						  try
						  {
								...
								else
								{
									 ...
									 else if (this.Actortype.Value==ActorType.Gizmo)
									 {
										  ...
										  } catch { Logging.WriteVerbose("Failure to get actor Gizmo Type!"); return false; }

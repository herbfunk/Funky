					 ...
					 foreach (Actor thisActor in ZetaDia.Actors.RActorList)
					 {
						  ...
						  using (ZetaDia.Memory.AcquireFrame())
						  {
								if (!ObjectCache.Objects.TryGetValue(tmp_raGUID, out tmp_CachedObj))
								{
									 ...
									 } catch { Logging.WriteVerbose("Failure to get SNO from object! RaGUID: {0}", tmp_raGUID); continue; }
									 #endregion
									 ...
									 } catch { Logging.WriteVerbose("Failure to get position vector for RAGUID {0}", tmp_raGUID); continue; }
									 ...
									 } catch { Logging.WriteVerbose("Failure to get ACDGUID for RAGUID {0}", tmp_raGUID); continue; }

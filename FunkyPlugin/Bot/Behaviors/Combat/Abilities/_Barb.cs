...
namespace FunkyTrinity
{
	 ...
	 {
		  internal static cacheSNOPower BarbAbility(bool bCurrentlyAvoiding=false, bool bOOCBuff=false, bool bDestructiblePower=false)
		  {
				...
				if (bDestructiblePower)
				{
					 ...
					 if (destructiblePower==SNOPower.Barbarian_WeaponThrow)
					 {
						  ...
						  {
								return new cacheSNOPower(SNOPower.Barbarian_WeaponThrow, 15f, vNullLocation, -1, -1, 0, 0, USE_SLOWLY);
						  }
						  else
						  {
				...
				if (!bOOCBuff&&!bCurrentlyAvoiding&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_Earthquake)&&!Bot.Character.bIsIncapacitated&&
					(Bot.Combat.iElitesWithinRange[RANGE_15]>0||((thisCacheUnitObj!=null&&thisCacheUnitObj.IsEliteRareUnique||Bot.Target.ObjectData.IsBoss)&&Bot.Target.ObjectData.RadiusDistance<=13f))&&
				...
				if (!bOOCBuff&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_WrathOfTheBerserker)&&
					...
					Bot.Target.ObjectData.SNOID!=193077&&
					 // Make sure we are allowed to use wrath on goblins, else make sure this isn't a goblin
					(SettingsFunky.Class.bGoblinWrath||!Bot.Target.ObjectData.IsTreasureGoblin)&&
					 // If on a boss, only when injured
					((Bot.Target.ObjectData.IsBoss&&thisCacheUnitObj!=null&&thisCacheUnitObj.CurrentHealthPct.Value<=0.99&&!HotbarAbilitiesContainsPower(SNOPower.Barbarian_Whirlwind))||
					 // If *NOT* on a boss, and definitely no boss in range, then judge based on any elites at all within 30 feet
					 ((!Bot.Target.ObjectData.IsBoss||
				...
				if (!bOOCBuff&&!bCurrentlyAvoiding&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_CallOfTheAncients)&&!Bot.Character.bIsIncapacitated&&
					(Bot.Combat.iElitesWithinRange[RANGE_25]>0||((thisCacheUnitObj!=null&&thisCacheUnitObj.IsEliteRareUnique||Bot.Target.ObjectData.IsTreasureGoblin||Bot.Target.ObjectData.IsBoss)&&Bot.Target.ObjectData.RadiusDistance<=25f))&&
				...
				if (!bOOCBuff&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_ThreateningShout)&&!Bot.Character.bIsIncapacitated&&
					(
						Bot.Combat.iElitesWithinRange[RANGE_20]>1||(Bot.Target.ObjectData.IsBoss&&Bot.Target.ObjectData.RadiusDistance<=20)||
				...
				if (!bOOCBuff&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_Revenge)&&!Bot.Character.bIsIncapacitated&&
					...
					(!Bot.Target.ObjectData.IsTreasureGoblin||Bot.Combat.iAnythingWithinRange[RANGE_12]>=5)&&
					 // Doesn't need CURRENT target to be in range, just needs ANYTHING to be within 9 foot, since it's an AOE!
					(Bot.Combat.iAnythingWithinRange[RANGE_6]>0||Bot.Target.ObjectData.RadiusDistance<=6f)&&
				...
				if (!bOOCBuff&&!bCurrentlyAvoiding&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_FuriousCharge)&&!Bot.Character.bIsRooted&&!Bot.Character.bIsIncapacitated&&
					(Bot.Combat.iElitesWithinRange[RANGE_25]>0||Bot.Combat.iAnythingWithinRange[RANGE_15]>3||Bot.Character.dCurrentHealthPct<=0.7||Bot.Target.ObjectData.CentreDistance>=15f||thisCacheUnitObj!=null&&thisCacheUnitObj.IsEliteRareUnique||Bot.Target.ObjectData.IsTreasureGoblin||Bot.Target.ObjectData.IsBoss)&&
					...
					PowerManager.CanCast(SNOPower.Barbarian_FuriousCharge))
				{
					 ...
					 if (Bot.Target.ObjectData.CentreDistance<=15)
						  fExtraDistance=10;
					 else
						  fExtraDistance=(25-Bot.Target.ObjectData.CentreDistance);
				...
				if (!bOOCBuff&&!bCurrentlyAvoiding&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_Leap)&&!Bot.Character.bIsIncapacitated&&
					...
					(Bot.Character.dCurrentHealthPct<=0.75||Bot.Target.ObjectData.CentreDistance>=18f||thisCacheUnitObj!=null&&thisCacheUnitObj.ForceLeap||Bot.Combat.iElitesWithinRange[RANGE_30]>=1)&&
					...
					PowerManager.CanCast(SNOPower.Barbarian_Leap))
				{
					 ...
					 float fExtraDistance=Bot.Target.ObjectData.Radius;
				...
				if (!bOOCBuff&&!Bot.Class.bWaitingForSpecial&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_Rend)&&!Bot.Combat.UsedAutoMovementCommand&&!Bot.Character.bIsIncapacitated&&
					...
					(!Bot.Target.ObjectData.IsTreasureGoblin&&(!SnoCacheLookup.hashActorSNOFastMobs.Contains(Bot.Target.ObjectData.SNOID)||thisCacheUnitObj!=null&&thisCacheUnitObj.IsEliteRareUnique)||Bot.Combat.iAnythingWithinRange[RANGE_6]>3)
					&&
					 //Non-WW users
					(!HotbarAbilitiesContainsPower(SNOPower.Barbarian_Whirlwind)&&
							(
					...
					// This segment is for people who *DO* have whirlwind
					 ...
					 // 3+ mobs of any kind at close range *OR* one elite/boss/special at close range
							 (
								...
								(Bot.Combat.iAnythingWithinRange[RANGE_15]>=3&&Bot.Target.ObjectData.IsTreasureGoblin&&Bot.Target.ObjectData.RadiusDistance<=6f)||
								Bot.Combat.iAnythingWithinRange[RANGE_15]>=5||
								((thisCacheUnitObj!=null&&thisCacheUnitObj.IsEliteRareUnique||Bot.Target.ObjectData.IsBoss)&&Bot.Target.ObjectData.RadiusDistance<=6f&&Bot.Combat.iAnythingWithinRange[RANGE_15]>=3)
							 )
						 )
					)&&
				...
				if (!bOOCBuff&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_Overpower)&&!Bot.Character.bIsIncapacitated&&
					...
					(
						...
						(Bot.Character.dCurrentHealthPct<=0.85&&Bot.Target.ObjectData.RadiusDistance<=5f)||
						(
							Bot.Combat.iAnythingWithinRange[RANGE_6]>=1&&
							((thisCacheUnitObj!=null&&thisCacheUnitObj.IsEliteRareUnique)||Bot.Target.ObjectData.IsBoss||HasBuff(SNOPower.Barbarian_WrathOfTheBerserker)||
					 ...
					 &&(Bot.Character.dCurrentEnergy>30||Bot.Class.RuneIndexCache[SNOPower.Barbarian_SeismicSlam]==3&&Bot.Character.dCurrentEnergy>15))
				{
					 ...
					 {
						  return new cacheSNOPower(SNOPower.Barbarian_SeismicSlam, 40f, vNullLocation, -1, thisCacheUnitObj.AcdGuid.Value, 2, 2, USE_SLOWLY);
					 }
					 else if (ObjectCache.Objects.Clusters(ClusterDist, 40f, 2).Count>0)
					 {
						  ...
						  return new cacheSNOPower(SNOPower.Barbarian_SeismicSlam, 40f, vNullLocation, -1, ACDGuid, 2, 2, USE_SLOWLY);
					 }
				}
				// Ancient spear 
					...
					PowerManager.CanCast(SNOPower.Barbarian_AncientSpear))
				{
					 ...
					 if (fExtraDistance<=4f)
						  fExtraDistance=4f;
					 if (Bot.Target.ObjectData.CentreDistance+fExtraDistance>30f)
						  fExtraDistance=30-Bot.Target.ObjectData.CentreDistance;
					 if (fExtraDistance<5f)
						  fExtraDistance=5f;
					 Vector3 vNewTarget=MathEx.CalculatePointFrom(Bot.Target.ObjectData.Position, Bot.Character.Position, Bot.Target.ObjectData.CentreDistance+fExtraDistance);
				...
				if (!bOOCBuff&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_Sprint)
					...
					(
					 ...
					 // Always keep up if we are whirlwinding, or if the target is a goblin
							(HotbarAbilitiesContainsPower(SNOPower.Barbarian_Whirlwind)||Bot.Target.ObjectData.IsTreasureGoblin))
					)&&
					 ...
					 &&!Bot.Character.bIsIncapacitated&&Bot.Character.dCurrentEnergy>=20&&PowerManager.CanCast(SNOPower.Barbarian_HammerOfTheAncients)&&!Bot.Class.bWaitingForSpecial)
				{
					 ...
					 if (Bot.Target.ObjectData.IsTreasureGoblin)
						  return new cacheSNOPower(SNOPower.Barbarian_HammerOfTheAncients, 12f, vNullLocation, -1, Bot.Target.ObjectData.AcdGuid.Value, 1, 3, USE_SLOWLY);
					 else
					 {
						  ...
						  {
								ACDGuid=Bot.Target.ObjectData.AcdGuid.Value;
								return new cacheSNOPower(SNOPower.Barbarian_HammerOfTheAncients, Range, vNullLocation, -1, ACDGuid, 1, 2, USE_SLOWLY);
						  }
					 }
				}
				// Weapon throw
				if (!bOOCBuff&&!bCurrentlyAvoiding&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_WeaponThrow)&&!Bot.Character.bIsIncapacitated&&
					Bot.Character.dCurrentEnergy>=10)
				{
					 return new cacheSNOPower(SNOPower.Barbarian_WeaponThrow, 44f, vNullLocation, -1, Bot.Target.ObjectData.AcdGuid.Value, 0, 1, USE_SLOWLY);
				}
				// Frenzy rapid-attacks
				if (!bOOCBuff&&!bCurrentlyAvoiding&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_Frenzy))
				{
					 return new cacheSNOPower(SNOPower.Barbarian_Frenzy, 10f, vNullLocation, -1, Bot.Target.ObjectData.AcdGuid.Value, 0, 0, SIGNATURE_SPAM);
				}
				// Bash fast-attacks
				if (!bOOCBuff&&!bCurrentlyAvoiding&&HotbarAbilitiesContainsPower(SNOPower.Barbarian_Bash))
				{
					 return new cacheSNOPower(SNOPower.Barbarian_Bash, 10f, vNullLocation, -1, Bot.Target.ObjectData.AcdGuid.Value, 0, 1, USE_SLOWLY);
				}
				// Cleave fast-attacks

		...
		public static bool HandleTargeting()
		{
			...
			if (!FunkyGame.Targeting.Cache.bWholeNewTarget && !FunkyGame.Targeting.Cache.bWaitingForPower && !FunkyGame.Targeting.Cache.bWaitingForPotion)
			{
				...
				if (bShouldRefreshDiaObjects)
				{
					...
					FunkyGame.Targeting.Movement.NewTargetResetVars();
				}
			...
			FunkyGame.Targeting.Movement.CurrentTargetLocation = FunkyGame.Targeting.Cache.CurrentTarget.Position;
			...
			if (ObjectCache.CheckTargetTypeFlag(FunkyGame.Targeting.Cache.CurrentTarget.targetType.Value, TargetType.LineOfSight | TargetType.Backtrack))
			{
				...
				if (Navigation.Navigation.NP.CurrentPath.Count > 0)
				{
					...
					else
					{
						...
						FunkyGame.Targeting.Movement.CurrentTargetLocation = Navigation.Navigation.NP.CurrentPath.Current;
					}

					FunkyGame.Targeting.Movement.TargetMoveTo(FunkyGame.Targeting.Cache.CurrentTarget);
					return true;
				}
			}
			...
			if (!FunkyGame.Targeting.Cache.CurrentTarget.WithinInteractionRange())
			{
				FunkyGame.Targeting.Movement.TargetMoveTo(FunkyGame.Targeting.Cache.CurrentTarget);
				return true;
			}
			...
			switch (FunkyGame.Targeting.Cache.CurrentTarget.targetType.Value)
			{
				...
				case TargetType.CursedChest:
					FunkyGame.Targeting.Cache.CurrentTarget.Interact();
					break;
		...
		public virtual bool Movement()
		{
			...
			if (ObjectCache.CheckTargetTypeFlag(FunkyGame.Targeting.Cache.CurrentTarget.targetType.Value, TargetType.LineOfSight | TargetType.Backtrack))
			{
				...
				if (Navigation.Navigation.NP.CurrentPath.Count > 0)
				{
					...
					CurrentState = FunkyGame.Targeting.Movement.TargetMoveTo(FunkyGame.Targeting.Cache.CurrentTarget);
					return false;
				}
			}

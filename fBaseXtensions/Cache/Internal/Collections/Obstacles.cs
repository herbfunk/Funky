...
namespace fBaseXtensions.Cache.Internal.Collections
{
	...
	public class ObstacleCollection : IDictionary<int, CacheObstacle>
	{
		...
		public bool TryGetValue(int key, out CacheObstacle value)
		{
			...
			if (found && value.RefreshRemovalCounter < 0)
			{
		...
		//		  &&(obs.CentreDistance-FunkyGame.Hero.Class_.Data.fCharacterRadius)<=maxRange //within maximum range!
		...
		public void AttemptToClearEntries()
		{
			if (RemovalList.Count > 0)
			{
				foreach (int item in RemovalList)
				{
					if (ContainsKey(item))
					{
						if (obstacles[item].RefreshRemovalCounter > 0 && obstacles[item].CreationTimeSeconds < obstacles[item].RefreshRemovalCounter)
						{
		...
		public bool TestVectorAgainstAvoidanceZones(Vector3 startingPoint, Vector3 destinationPoint, bool IgnoreTriggeringAvoidances = true)
		{
			return Avoidances.Any(A => A.ShouldAvoid &&
				  (!IgnoreTriggeringAvoidances || !FunkyGame.Targeting.Cache.Environment.TriggeringAvoidanceRAGUIDs.Contains(A.RAGUID)) &&
		...
		//{
		//	 Vector3 currentPosition=FunkyGame.Hero.Class_.Data.Position;

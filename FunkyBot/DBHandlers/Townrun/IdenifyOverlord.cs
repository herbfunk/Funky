...
namespace FunkyBot.DBHandlers
{
	...
	internal static partial class TownRunManager
	{
		...
		private static void RefreshUnidList(bool behaviorRefresh = false)
		{
			...
			{
				Logger.DBLog.DebugFormat("Exception occured during refresh of inventory");
			}
		}
		...
		internal static RunStatus IdenifyItemManualBehavior(object ret)
		{
			...
			if (UnidentifiedItems.Count > 0)
			{
				...
				if (totalFailures > 2)
				{
					Logger.DBLog.DebugFormat("Quiting Idenify Behavior due to too many failures");
					return RunStatus.Success;
				}
			...
			Logger.DBLog.DebugFormat("[Funky] Idenify Behavior Finishing!");
		...
		internal static RunStatus IdenifyItemManualFinishBehavior(object ret)
		{
			...
			townRunItemCache.IdenifyItems.Clear();

			Logger.DBLog.DebugFormat("[Funky] Idenify Behavior Finished!");
			return RunStatus.Success;
		}
		...
		internal static RunStatus IdenifyItemBookOfCainMovementBehavior(object ret)
		{
			...
			Vector3 vectorBookOfCainLocation = new Vector3(0f, 0f, 0f);
			...
			{
				vectorBookOfCainLocation = ReturnMovementVector(TownRunBehavior.Idenify, curAct);
			}
			else
			...
			Logger.DBLog.DebugFormat("[Funky] Idenify Behavior Book of Cain Movement Finished!");
			return RunStatus.Success;
		}
		...
		internal static RunStatus IdenifyItemBookOfCainInteractionBehavior(object ret)
		{
			...
			if (UnidentifiedItems.Count == 0)
			{
				Logger.DBLog.DebugFormat("[Funky] Idenify Behavior Book of Cain Interaction Finished!");
				return RunStatus.Success;
			}

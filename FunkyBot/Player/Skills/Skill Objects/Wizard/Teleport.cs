...
namespace FunkyBot.Skills.Wizard
{
	public class Teleport : Skill
	{
		...
		public override void Initialize()
		{
			...
			FcriteriaCombat = () => ((Bot.Settings.Wizard.bTeleportFleeWhenLowHP && FunkyGame.Hero.dCurrentHealthPct < 0.5d)
								   ||
								   (Bot.Settings.Wizard.bTeleportIntoGrouping &&
									Bot.Targeting.Cache.Clusters.AbilityClusterCache(combatClusterCondition).Count > 0 &&
									Bot.Targeting.Cache.Clusters.AbilityClusterCache(combatClusterCondition)[0].Midpoint.Distance(FunkyGame.Hero.Position) > 15f)
			...
			{
				float fDistanceFromTarget = FunkyGame.Hero.Position.Distance(v);
				if (Funky.Difference(FunkyGame.Hero.Position.Z, v.Z) <= 4 && fDistanceFromTarget >= 10f)
				{
					if (fDistanceFromTarget > 50f)
						return MathEx.CalculatePointFrom(v, FunkyGame.Hero.Position, 50f);
					return v;

	...
	public class Skill : SkillCriteria
	{
		...
		public virtual void OnSuccessfullyUsed(bool reorderAbilities = true)
		{
			...
			if (Bot.Settings.Backtracking.EnableBacktracking && Bot.Settings.Backtracking.TrackStartOfCombatEngagment && Bot.Targeting.Cache.StartingLocation.Equals(Vector3.Zero))
				Bot.Targeting.Cache.StartingLocation = FunkyGame.Hero.Position;
			...
			if (Destructible)
			{
				...
				if (LocationalAttack)
				{
					...
					if (!ability.IsRanged)
					{
						...
						attacklocation = MathEx.GetPointAt(FunkyGame.Hero.Position, 0.50f, Navigation.FindDirection(FunkyGame.Hero.Position, Bot.Targeting.Cache.CurrentTarget.Position, true));
					}
					else
					{
						attacklocation = MathEx.GetPointAt(Bot.Targeting.Cache.CurrentTarget.Position, 1f, Navigation.FindDirection(Bot.Targeting.Cache.CurrentTarget.Position, FunkyGame.Hero.Position, true));
					}
